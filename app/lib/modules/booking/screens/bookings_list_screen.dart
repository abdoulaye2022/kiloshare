...
class _BookingsListScreenState extends State<BookingsListScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadBookings() async {
    ...
    // Maintenant récupérer le token valide (peut déclencher refresh si nécessaire)
  ...
  Future<void> _rejectBooking(BookingModel booking) async {
    final result = await showDialog<Map<String, dynamic>>(
      context: context,
      builder: (context) => _RejectBookingDialog(booking: booking),
    );
    ...
    if (result != null && result['confirmed'] == true) {
      ...
      try {
        final rejectionReason = result['reason'] as String?;
        final rejectResult = await _bookingService.rejectBooking(
          booking.id.toString(),
...
// Dialog pour rejeter une réservation avec raison optionnelle
class _RejectBookingDialog extends StatefulWidget {
  final BookingModel booking;

  const _RejectBookingDialog({required this.booking});
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                hintText: 'Ex: Dates incompatibles, poids trop important, etc.',
