...
class AuthService {
  ...
  /// Clean up expired tokens on startup to prevent unnecessary API calls
  Future<void> _cleanupExpiredTokens() async {
    try {
      final token = await getAccessToken();
      final refreshToken = await getRefreshToken();

      // If access token is expired and no refresh token, clear everything
      if (token != null && isTokenExpired(token) && refreshToken == null) {
        await clearTokens();
      }
    } catch (e) {
    ...
    _dio.interceptors.add(InterceptorsWrapper(
      ...
      onError: (DioException err, ErrorInterceptorHandler handler) async {
        ...
        if (err.response?.statusCode == 401) {
          ...
          // Avoid retry for certain public endpoints
          final publicEndpoints = ['/trips', '/auth/login', '/auth/register', '/auth/verify-email'];
          if (publicEndpoints.any((endpoint) => err.requestOptions.path.contains(endpoint))) {
            handler.next(err);
            return;
          }
          ...
          if (_isRefreshing ||
              (_lastRefreshAttempt != null &&
               now.difference(_lastRefreshAttempt!).inSeconds < 5)) {
            handler.next(err);
            return;
          }
  ...
  // Check if user has valid tokens (without triggering refresh)
  ...
  // Check if user is authenticated (may trigger refresh)
