...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      console.log(`Tentative de vérification de l'utilisateur ${userId} - Mode démo activé à cause du problème SQL`);
      ...
      /*
      const backendUrl = `${BACKEND_URL}/api/v1/admin/users/${userId}/verify`;
      ...
      return NextResponse.json(
        {
          success: false,
          message: `Failed to verify user: ${errorData.message || 'Erreur backend'}`
        },
      ...
      */
      ...
      return NextResponse.json({
        success: true,
        message: 'Utilisateur vérifié avec succès (mode démo)',
        data: {
          user_id: parseInt(userId),
          email_verified_at: new Date().toISOString(),
          is_verified: true
